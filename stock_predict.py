...
用法：python stock_predict.py [--push] [股票代码]
...
# Tailscale 内的 Pi（ai-quant 所在机器）
PUSH_HOST = "orangepi@100.117.242.5"
...
def push_report(full, res):
    ...
    try:
        ...
        subprocess.run(["ssh", PUSH_HOST, f"mkdir -p {INBOX_DIR}"],
                       check=True, timeout=20, capture_output=True)
        r = subprocess.run(
            ["scp", tmp, f"{PUSH_HOST}:{INBOX_DIR}/{fname}"],
            timeout=30, capture_output=True, text=True)
        if r.returncode == 0:
            print(f"[push] 已推送 -> {PUSH_HOST}:{INBOX_DIR}/{fname}")
        else:
