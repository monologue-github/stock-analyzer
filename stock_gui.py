...
def get_daily(full: str, min_bars: int = 100, tail=None):
    ...
    if rows:
        try:
            remote = [r for r in _fetch_remote_rows(full, count=900)
...
def _pool_match(pool_rows, cur, vr_now, idx_chg_by_date, idx_chg_today,
    ...
    for code, rows in pool_rows:
        ...
        last_i = len(rets) - W
        for i in range(W, last_i + 1):
            w = znorm(rets[i-W:i])
            d_px = math.sqrt(sum((a-b)**2 for a,b in zip(cur,w)))
            vr_i = vr_arr[i]
            ...
            sims.append((d_px+s_v+s_i+d_x,i,code,d_px,s_v,s_i))
    if not info:
        return []
...
def analyze(full, progress=None, quick=False):
    ...
    for idx_i, day, sc, reasons in _bull_scores:
        ...
        # 弱势行情过滤：当日大盘弱势时 BUY 加严；买入阈值/冷却按风险偏好
        rp = CFG.risk_params()
        ...
        if sc >= buy_threshold and prev_dir <= 0:
            ...
            prev_dir = 1
            cooldown = CFG.risk_params()["cooldown"]
...
class App:
    ...
    def _draw_main(self):
        ...
        for i, b in enumerate(bars):
            ...
            if isp:
                continue            # 预测K线已删除：点预测无alpha，不画不丢人
            up = b["close"] >= b["open"]
